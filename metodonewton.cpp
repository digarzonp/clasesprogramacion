...
double newtonmethod (double xl, double xu, double eps, int nmax, fptr h,fptr i);
;
int main(void)
{
  ...
  xr = newtonmethod(xl, xu, eps, NMAX, h,i);
  return 0;
}
...
double newtonmethod(double xl, double xu, double eps, int nmax, fptr func, fptr g)
{
  std::cout.precision(16);
  std::cout.setf(std::ios::scientific);
 double xr ;
  ...
  for(int ii = 0; ii <= nmax; ++ii) {
    xr = xl-(h(xl)/i(xl));
    std::cout << ii << "  " << xl << " " << xr << " " << h(xl) <<" "<<h(xl)<< std::endl;
    if (std::fabs(f(xr)) < eps) break;
    else
      xl=xr;
  }
